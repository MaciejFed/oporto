...
export async function getKnownPercentage(language: Language): Promise<number> {
  ...
  const sortedPairCounts = Object.entries(pairCounts).sort((a, b) => b[1] - a[1]);
  const sortedThirdCounts = Object.entries(thirdsCounts).sort((a, b) => b[1] - a[1]);
  const sortedForthCounts = Object.entries(forthCounts).sort((a, b) => b[1] - a[1]);
