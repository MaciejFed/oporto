...
function getAllUniqueWords() {
  ...
  const allWords = [nouns, verbs, sentenceWords, fitInWords].flatMap((w) => w).map((word) => word.replace('?', '').toLowerCase()).filter((verb) => !verbDecliatons.includes(verb)).filter((word) => word).sort();

  const x = [...new Set(allWords)];
  console.log(x);
}
